  ...
  return (
    <div className="flex flex-col gap-4 ">
      {storeData.map((store) => (
        <Card
          key={store.id}
          className="w-full bg-white shadow-md p-4 rounded-lg relative"
        >
          ...
          {user?.role === "systemAdmin" || user?.role === "storeOwner" ? (
            <div className="absolute top-2 right-2 max-sm:text-sm">
